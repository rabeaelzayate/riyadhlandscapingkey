...
import { Phone, MessageCircle, ArrowLeft, Send } from "lucide-react";
    ...
    <section id="contact" className="py-20 bg-gradient-to-br from-emerald-950 via-emerald-900 to-emerald-950 text-white relative overflow-hidden">
      ...
      <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <p className="text-base sm:text-lg text-emerald-100/90 leading-relaxed max-w-2xl mx-auto mb-10">
          تواصل مع مؤسسة {company.nameAr} اليوم واحصل على الحل المناسب والمعاينة لمساحة فيلتك أو حديقتك.
        </p>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-6">
          <WhatsAppButton
            size="lg"
            variant="gold"
            message={`مرحبًا، أرغب في طلب معاينة مجانية لموقعي لدى مؤسسة ${company.nameAr}.`}
          >
            اطلب معاينة مجانية
          </WhatsAppButton>
          ...
          <Link
            href="/contact/"
            className="inline-flex items-center justify-center gap-2.5 px-7 py-3.5 rounded-2xl bg-white text-emerald-950 font-bold hover:bg-emerald-50 transition-all shadow-lg"
          >
            <Send className="w-4 h-4 text-emerald-700" />
            <span>صفحة حجز واستشارة مجانية</span>
          </Link>

          {company.phone && (
            <a
              href={`tel:${company.phone}`}
              className="inline-flex items-center justify-center gap-2.5 px-6 py-3.5 rounded-2xl border-2 border-emerald-400 text-emerald-100 font-bold hover:bg-emerald-800/60 transition-colors"
            >
